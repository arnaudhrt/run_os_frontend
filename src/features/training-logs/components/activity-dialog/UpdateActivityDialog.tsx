...
import { useUpdateActivityStore } from "../../stores/update-activity.store";
...
interface UpdateActivityDialogProps {
  ...
  onSubmit: (data: UpdateActivityParams) => void;
  loading?: boolean;
}

export function UpdateActivityDialog({ activity, open, onOpenChange, onSubmit, loading }: UpdateActivityDialogProps) {
  const {
    step,
    direction,
    activityType,
    ...
    reset,
    initFromActivity,
  } = useUpdateActivityStore();

  useEffect(() => {
    if (open && activity) {
      initFromActivity(activity);
    }
  }, [open, activity, initFromActivity]);
  ...
  const canProceed = () => {
    switch (step) {
      case 0:
        return activityType !== undefined && (workoutType !== undefined || activityType === "hike");
      case 1:
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-4 max-h-[300px] overflow-y-auto scrollable pr-2">
                    ...
                    <div className="space-y-2">
                      ...
                      {hasPain && (
                        <NativeSelect value={painLocation || ""} onChange={(e) => setPainLocation(e.target.value as Pain)} className="w-full">
